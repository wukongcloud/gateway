...
var (
	xdsSnapshotCreateTotal = metrics.NewCounter(
		"xds_snapshot_create_total",
		"Total number of xds snapshot cache creates.",
	)

	xdsSnapshotUpdateTotal = metrics.NewCounter(
		"xds_snapshot_update_total",
		"Total number of xds snapshot cache updates by node id.",
	)

	xdsStreamDurationSeconds = metrics.NewHistogram(
		"xds_stream_duration_seconds",
		"How long a xds stream takes to finish.",
